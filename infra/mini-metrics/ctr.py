...
Shared by mini-metrics (local docker) and dgx-scrape (DGX docker over SSH) — both
run on the mini, so one helper serves both boxes. Reads the inspect stream on
stdin, writes prometheus lines on stdout.

Input (one container per line, tab-separated), produced by:
    docker inspect $(docker ps -aq) \
      --format '{{.Name}}\t{{.State.Status}}\t{{.State.StartedAt}}\t{{index .Config.Labels "com.docker.compose.project"}}\t{{json .NetworkSettings.Ports}}'
