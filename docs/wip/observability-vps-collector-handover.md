...
- **VictoriaMetrics** (ingest) on the DGX: `http://100.69.49.126:8428/api/v1/write`
  - `100.69.49.126` = DGX (`dgx-llm-1`) Tailscale IP. Reachable from any tailnet host.
  - No auth on the tailnet.
- **Grafana** (where you'll verify): `http://100.69.49.126:3000` (Homelab folder).
...
```sh
REMOTE_WRITE_URL=http://100.69.49.126:8428/api/v1/write   # DGX VictoriaMetrics
...
curl -s "http://100.69.49.126:8428/api/v1/query?query=count({instance='prod-podcast'})"
```
...
- Ports bind to the **tailnet IP**, so verify with `100.69.49.126`, never
  `127.0.0.1`.
